...
const SkillsDev: React.FC = () => {
  ...
  const currentSkills = dataLeng.skillsDev.techSkills[activeTab];
  ...
  return (
    <section className={`${UI_COLORS.container} rounded-xl p-4 space-y-4 overflow-hidden`}>
      <header>
        <h2 className="text-3xl font-bold text-center uppercase">{dataLeng.skillsDev.header.title}</h2>
        <h3 className="px-4 md:px-10">{dataLeng.skillsDev.header.subtitle}</h3>
      </header>
      ...
      <div>
        {currentSkills.length > 0 ? (
              ...
              >
                <section className="space-y-4">
                  {skill.levels.map((level, index) => (
                    ...
                    >
                      <h3 className={`border-b-4 border-dashed ${tones.borderColor.normal} text-3xl uppercase font-bold`}>{level.name}</h3>

                      <ul className="space-y-2 px-6 list-disc">
                        {level.lessons.map((item, index) => (
                          <li key={index} className="text-sm">
                            <h5><span className={`${tones.bgColor.dark} py-1 px-2 ${tones.textColor}`}><strong className="uppercase">{item.name}</strong></span></h5>
          ...
          <p className="text-center">{dataLeng.skillsDev.noSkills}</p>
        )}
      </div>
    </section>
  );
