...
const Card: React.FC<{ card: Card; onClick: () => void }> = ({ card, onClick }) => {
  const { tones } = useThemeController();
  return (
    <div
      className={`${card.isFlipped ? tones.bgColor.dark : 'bg-stone-800 hover:bg-stone-700'} rounded-xl flex items-center justify-center p-4`}
      onClick={onClick}
      role="button"
  ...
  return (
    <>
      <section className={`${UI_COLORS.container} rounded-xl p-2 md:p-4 space-y-4`}>
        <header className={`border-b-4 ${tones.borderColor.normal} pb-4 flex flex-col md:flex-row justify-between items-center gap-4`}>
          ...
          <div className={`md:border-l-4 ${tones.borderColor.normal} md:pl-4 flex justify-between items-center w-full`}>
            ...
            <Button onClick={initializeGame} text="reiniciar el juego" rightIcon={<IconRestartAlt />} />
          </div>
        </header>
